...
import * as styles from './AnalysisLoadingScreen.css';
...
const WARNING_MSG = '분석에 시간이 걸리고 있어요. 조금만 더 기다려 주세요.';
...
export const AnalysisLoadingScreen = ({ companyName, fileCount, onCancel }: Props) => {
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setProgress({ value: 15, duration: 3000, easing: 'ease-out' }), 100);
    const t2 = setTimeout(() => setProgress({ value: 40, duration: 20000, easing: 'linear' }), 5000);
    const t3 = setTimeout(() => setProgress({ value: 65, duration: 30000, easing: 'linear' }), 25000);
    const t4 = setTimeout(() => setProgress({ value: 82, duration: 40000, easing: 'linear' }), 55000);
  ...
  useEffect(() => {
    const t = setTimeout(() => setShowWarning(true), 90000);
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.footer}>
        <span className={styles.footerText}>보통 1~2분 정도 소요돼요</span>
